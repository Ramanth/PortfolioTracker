...
def create_sub_portfolio_table():
    ...
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS sub_portfolio (
...
def insert_record(stock, quantity, avg_price):
    ...
    cursor = connection.cursor()
    cursor.execute('''
    INSERT OR REPLACE INTO sub_portfolio (Stock, Quantity, AvgPrice)
    VALUES (?, ?, ?)
...
# insert_record('HAPPSTMNDS', 50, 847)
