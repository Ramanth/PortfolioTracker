...
def create_sub_portfolio_table():
    ...
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS waya_portfolio (
...
def insert_record(stock, quantity, avg_price):
    ...
    cursor = connection.cursor()
    cursor.execute('''
    INSERT OR REPLACE INTO waya_portfolio (Stock, Quantity, AvgPrice)
    VALUES (?, ?, ?)
...
# insert_record('CIEINDIA', 85, 472)
# insert_record('ELGIEQUIP', 35, 509)
# insert_record('AARTIIND', 40, 482)
# insert_record('FIVESTAR', 40, 742)
# insert_record('LATENTVIEW', 50, 416)
# insert_record('BHARTIARTL', 0, 0)
# insert_record('ICICIBANK', 35, 941)
